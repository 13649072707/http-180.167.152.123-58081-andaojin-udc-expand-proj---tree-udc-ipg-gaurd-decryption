...
layui.define(['lay', 'element', 'form'], function (exports) { //假如该组件依赖 layui.form
	...
	Class.prototype.getTabContentChildElem = function (index) {
		var that = this,
			...
			, data = function () {
				...
				if (cron[tabItemKey] === '*' || cron[tabItemKey] === '0') {
					// 每次
					return {
						type: 'every',
						value: '*'
				...
				if (cron[tabItemKey].trim(' ') === '?' || cron[tabItemKey] === undefined || cron[tabItemKey] === '') {
					// 不指定
					return {
						type: 'unspecified',
		...
		var elem = lay.elem('div', {
			...
			'class': 'layui-tab-item layui-form ' + (index === 1 ? SHOW : "")
	...
	Class.prototype.Reset = function (valueArr) {
