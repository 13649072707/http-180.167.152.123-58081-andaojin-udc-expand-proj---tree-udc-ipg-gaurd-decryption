...
layui.define('form', function (exports) {
  ...
  Class.prototype.render = function () {
    ...
    options.reElem = $('<div class="layui-unselect layui-form-select">' +
        ...
        '<input type="hidden"  value="" readonly="" class="layui-input layui-unselect">' +
    ...
    options.el.on('click', '.layui-select-title', function () {
      ...
      if (!$dd0.hasClass('layui-hide')) {
        ...
        $title.find('input[type="hidden"]').val(options.values.join(options.valueSeparator));
        $title.find('input[type="text"]').val(options.labelValues.join(options.valueSeparator));
    })
  ...
  Class.prototype.formatData = function (data) {
    ...
    layui.each(data, function (i, item) {
      ...
      data[i][indexName] = i;
        if (data[i][checkedName]) {
            console.log(false + "" + checkedName)
            data[i][checkedName] = false;
        }
        layui.each(seleceedVal, function (index, value) {
  ...
  // 渲染数据
    Class.prototype.renderData = function (data) {
        ...
        var data = data ? that.formatData(data) : that.formatData(options.data);
        console.log(data)
    items = {
      ...
      // 单选
        radio: function (config, data, id) {            
          ...
          var elID = $(config.el),
            el = elID.find('dl'),
            valueName = config.valueName,
            showText = config.showText || config.valueName,
          checkedName = config.config.checkedName,
          indexName = config.config.indexName,
        ...
        layui.each(data, function (i, item) {
          ...
          showText = showText || valueName;
        // 渲染单选框
        el.find('dd').eq(0).nextAll().each(function (index) {
            var $dd = $(this),
                item = data[index],
                layuiValue = item[valueName],
                labelVal = item[showText];
            //title = layuiValue;
          if (label.length > 0) {
            title = "";
            layui.each(label, function (i, n) {
            ...
            dd.prop('checked', true);
                values.push(layuiValue);
                labelValues.push(title);
                $dd.parent().prev().find("input[type='hidden']").val(values.join(valueSeparator))
                $dd.parent().prev().find("input[type='text']").val(labelValues.join(valueSeparator))
          }
          $dd.html(dd);
        })
    ...
    if (type === 'checkbox') {
      ...
      ele.parents('dl').find('[type="checkbox"]').each(function (i) {
        var $dd = $(this),
          ...
          checked && ywIndex ? options.labelValues.push($dd.attr('title')) : "";
      })
        ele.parent().prev().find('input[type="hidden"]').val(options.values.join(options.valueSeparator));
        ele.parent().prev().find('input[type="text"]').val(options.labelValues.join(options.valueSeparator));
      ...
      layui.event.call(ele, MOD_NAME, MOD_NAME + '(' + filter + ')', {
        ...
        var index = ele.find('input').attr('yw-index'),
            ...
            showValue = ele.find('input').attr('title');
      options.values = [value];
        ele.parent().prev().find('input[type="hidden"]').val(value);
        ele.parent().prev().find('input[type="text"]').val(showValue);
