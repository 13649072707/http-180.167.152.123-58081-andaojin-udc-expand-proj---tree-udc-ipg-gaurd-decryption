...
ah.proxy({
    ...
    onRequest: (config, handler) => {
        var token = getCookie('dockingtoken');
        if (token) {
    ...
    onResponse: (response, handler) => {
        if (response.status === 401)//未登录就跳转登录
        {
            if (window.top) {
                window.top.location = "/docking/login.html";
                return;
            }
            location.href = "./login.html";
            return;
        }
        if (response.config.url.indexOf('.')<=-1  && response.status === 200) {
