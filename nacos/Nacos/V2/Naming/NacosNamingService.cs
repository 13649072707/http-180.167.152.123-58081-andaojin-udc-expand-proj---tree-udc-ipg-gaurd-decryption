namespace Nacos.V2.Naming
{
    ...
    public class NacosNamingService : INacosNamingService
    {
        ...
        public async Task UpdateInstance(string serviceName, Instance instance)
          => await _clientProxy.UpdateInstance(serviceName,Constants.DEFAULT_GROUP, instance).ConfigureAwait(false);

       public  async Task<Service> QueryService(string serviceName, string groupName)
            =>await _clientProxy.QueryService(serviceName, groupName).ConfigureAwait(false);
    }
}
