namespace Nacos.V2.Naming.Remote
{
    ...
    public class NamingClientProxyDelegate : INamingClientProxy, IDisposable
    {
        ...
        public async Task<Service> QueryService(string serviceName, string groupName) 
            => await GetExecuteClientProxy().QueryService(serviceName,groupName).ConfigureAwait(false);
        ...
        public async Task UpdateInstance(string serviceName, string groupName, Instance instance)
            => await httpClientProxy.UpdateInstance(serviceName,groupName,instance).ConfigureAwait(false);
