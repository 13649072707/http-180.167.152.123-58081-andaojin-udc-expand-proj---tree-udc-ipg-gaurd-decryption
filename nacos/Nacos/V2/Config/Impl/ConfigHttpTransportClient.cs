namespace Nacos.V2.Config.Impl
{
    ...
    public class ConfigHttpTransportClient : AbstConfigTransportClient
    {
        ...
        protected override async Task<string> QueryConfig(int pageNo,int pageSize,string tenant,long readTimeout, bool notify)
        {
            try
            {
                ...
                paramters["dataId"] = "";
                paramters["group"] = "";
                paramters["pageNo"] = pageNo.ToString();
                paramters["pageSize"] = pageSize.ToString();
                paramters["search"] = "accurate";
                ...
                HttpResponseMessage result = await HttpGet(Constants.CONFIG_CONTROLLER_PATH, headers, paramters, _agent.GetEncode(), readTimeout).ConfigureAwait(false);
