...
namespace EDoc2.Api
{
    ...
    public class XnRestfulResultProvider : IUnifyResultProvider
    {
        ...
        public IActionResult OnException(ExceptionContext context, ExceptionMetadata metadata)
        {
            ...
            exceptionMetadata = UnifyContext.GetExceptionMetadata(context);

            var result = new XnRestfulResult<object>
            {
            ...
            if (exceptionMetadata.ErrorCode != null)
            {
                if (exceptionMetadata.ErrorCode is ErrorCodes)
                {
                    ErrorCodes errorCodes = (ErrorCodes)exceptionMetadata.ErrorCode;
                    result.Code = (int)errorCodes;
                }
                else
                {
                    result.Code = exceptionMetadata.ErrorCode;
                }
            }
